  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "assistant",
      content:
        "Select an AME event and tell me what you want to investigate. I will ask for the minimum scope needed before searching Splunk.",
    },
  ]);
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="secondary-button" onClick={() => void loadEvents()}>
            Refresh
          </button>
        </div>
      </header>
      ...
      <div className="layout">
        <aside className="panel sidebar">
          ...
          {error && (
            <div className="error-box">
              {error}
              <p>For local UI testing, set DEMO_MODE=true.</p>
            </div>
          )}
          ...
          <div className="event-list">
            {events.map((event) => (
              <button
                key={event.id}
                className={
                  "event-row " + (selectedEvent?.id === event.id ? "selected" : "")
                }
          ...
          <div className="panel scope-panel">
            ...
            {scope ? (
              <div className="scope-grid">
                ...
                <div><span className="label">Data sources</span><span>{scope.dataSources || "To be selected by the investigator"}</span></div>
              </div>
            ) : (
              <div className="empty">Scope will be established through the clarification questions.</div>
            )}
          </div>
          ...
          <div className="panel chat-panel">
            ...
            {questions.length > 0 && (
              <div className="questions">
                <div className="questions-title">Clarify the investigation</div>
        ...
        <aside className="panel rightbar">
          ...
          {searches.length === 0 ? (
            <div className="empty">
              The bot will not run searches until the investigation scope is clear.
            </div>
          ) : (
