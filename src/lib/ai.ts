...
const CLARIFICATION_PROMPT = [
  ...
  "Ask only questions that materially reduce search volume or resolve an ambiguity.",
...
const clarificationTool = {
  ...
  description: "Ask the analyst for only the missing information needed to narrow the investigation.",
  strict: true,
  parameters: {
    type: "object",
...
const readyTool = {
  ...
  description: "Declare that enough scope exists to begin read-only Splunk investigation.",
  strict: true,
  parameters: {
    type: "object",
    additionalProperties: false,
...
const searchTool = {
  type: "function",
  name: "search_splunk",
  description: "Run a read-only Splunk SPL search within the approved investigation scope. Prefer aggregation/tstats queries before raw-event drill-down.",
  strict: true,
  parameters: {
    type: "object",
    additionalProperties: false,
    properties: {
      ...
      earliest: { type: "string", description: "Search start time inside approved scope." },
      latest: { type: "string", description: "Search end time inside approved scope." },
...
export async function planInvestigation(messages: ChatMessage[], eventContext?: Record<string, unknown>): Promise<InvestigationPlan> {
  ...
  if (clarification?.arguments) {
    ...
    return normalizePlan(
      { status: "clarification_needed", scope: scopeFromUnknown(args.scope), questions: questionsFromUnknown(args.questions) },
      eventContext,
    );
  }
  ...
  return normalizePlan(
    {
      ...
      scope: { objective: "", target: "", earliest: "", latest: "", dataSources: "", focus: "" },
      questions: [{
        id: "scope",
...
export async function investigate(
  ...
  let response = await callAI(
    ...
    env.openAiModel,
    [
      { role: "developer", content: SYSTEM_PROMPT + scopePrompt + context },
      ...messages.map((message) => ({ role: message.role, content: message.content })),
    ],
    [searchTool],
  );
    ...
    for (const call of calls.slice(0, maxToolCalls - toolCalls)) {
      ...
      try {
        const args = JSON.parse(call.arguments) as {
          query: string; earliest: string; latest: string; reason: string;
        };
