...
export async function POST(request: NextRequest) {
  try {
    ...
    if (plan.status === "clarification_needed") {
      const questionsText = plan.questions.map((question, index) => {
      ...
      return NextResponse.json({
        ...
        message: { role: "assistant", content: "Before I search Splunk, I need to narrow the investigation so I do not scan unnecessary data.\n\n" + questionsText },
        questions: plan.questions, scope: plan.scope, searches: [], skills: [], demo: env.demoMode,
      });
    }

    if (env.aiProvider === "mock") return NextResponse.json({
      status: "ready",
      message: { role: "assistant", content: "The scope is specific enough to investigate. Mock mode does not execute live AI searches. Configure AI_PROVIDER=openai and OPENAI_API_KEY to run the investigator." },
      questions: [], scope: plan.scope, searches: [], skills: [], demo: env.demoMode,
    });

    const result = await investigate(messages, body.eventContext, plan.scope);
    return NextResponse.json({ status: "investigating", message: result.message, questions: [], scope: plan.scope, searches: result.searches, skills: result.skills, demo: env.demoMode });
  } catch (error) {
